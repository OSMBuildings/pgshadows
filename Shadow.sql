...
CREATE OR REPLACE FUNCTION ST_Shadow(sunpos sunpos, geom geometry, height decimal) RETURNS geometry
AS $$
...
BEGIN
  length = TAN(sunpos.y);
  v = ROW(COS(sunpos.x) / length, SIN(sunpos.x) / length);

  src_line = ST_ExteriorRing(geom);

  geometers = ST_GeoMeters(ST_Y(ST_Centroid(src_line)));
